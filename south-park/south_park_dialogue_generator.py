...
path = "data/south_park_input.txt"
char_idx_file = 'data/char_idx.pickle'

if not os.path.isfile(path):
    urllib.request.urlretrieve("https://s3-ap-southeast-2.amazonaws.com/tf-training-data/south-park-dialogue.txt", path)
...
m = tflearn.SequenceGenerator(g, dictionary=char_idx,
                              seq_maxlen=maxlen,
                              clip_gradients=5.0,
                              checkpoint_path='data/model_south_park')

for i in range(50):
    num_test_chars = 600
    seed = random_sequence_from_textfile(path, maxlen)
    m.fit(X, Y, validation_set=0.1, batch_size=128,
          n_epoch=1, run_id='south_park')
    print("-- TESTING...")
